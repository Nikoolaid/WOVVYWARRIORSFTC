...
public class Bologna extends LinearOpMode {
    ...
    protected double myOpen = 0.7;
    protected double myClosed = 0.2;
    ...
    public void runOpMode() {
        ...
        while (opModeIsActive()) {
            ...
            double r = Math.hypot(deadband(gamepad1.left_stick_x, .03), deadband(-gamepad1.left_stick_y, .03));
            double robotAngle = Math.atan2(deadband(-gamepad1.left_stick_y, .03), deadband(gamepad1.left_stick_x, .03)) - Math.PI / 4;
            ...
            // Weapons Things
            // Claw Controls
            if (deadband(gamepad2.left_trigger, .03) > 0) {
                leftTriggerGo = true;
            } else {
                leftTriggerGo = false;
            }
            if (deadband(gamepad2.right_trigger, .03) > 0) {
                rightTriggerGo = true;
            } else {
                rightTriggerGo = false;
            }
            ...
            telemetry.addData("Claw Servo Degrees:", "(%.3f) (%.3f)", clawRight.getPosition(), clawLeft.getPosition());
            //telemetry.addData("Ticks moved :", "(%.2f)", frontRight.getCurrentPosition());
            //telemetry.addData("Right trigger go :", "(%f)", rightTriggerGo);
            telemetry.update();    
        }
    }
